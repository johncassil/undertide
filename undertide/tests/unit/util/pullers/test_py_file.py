# user_function_str = """from datetime import datetime
# def find_file(client, bucket: str):
#     ## Find the most recent file in the bucket that ends with .csv
#     files = client.list_objects(Bucket=bucket)["Contents"]
#     files = [f["Key"] for f in files if f["Key"].endswith(".csv")]
#     files = sorted(files, key=lambda x: datetime.strptime(x.split("_")[-1], "%Y%m%d.csv"))
#     return files[-1]
# """
...
class TestUndertidePyFileFinder(unittest.TestCase):
    def test_get_file_with_s3_bucket_and_csv_format(self):
        ...
        with patch("boto3.client", return_value=s3_client_mock):
            with patch.object(file_finder, "transform_file", return_value="path/to/transformed_file.csv") as mock_transform_file:
                file_path = file_finder.get_file()
                mock_transform_file.assert_not_called()
        self.assertEqual(file_path, f"test_report_{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.csv")        
    ...
    def test_get_file_with_gcs_bucket_and_parquet_format(self):
        ...
        with patch("google.cloud.storage.Client", return_value=gcs_client_mock):
            with patch.object(file_finder, "transform_file", return_value="path/to/transformed_file.parquet") as mock_transform_file:
                file_path = file_finder.get_file()
                mock_transform_file.assert_not_called()
        self.assertEqual(file_path, f"test_report_{datetime.now().strftime('%Y-%m-%d-%H-%M-%S')}.parquet")
        

    def test_transform_file_with_csv_file(self):
        # define the input and expected output file names
        input_file_name = "path/to/input_file.csv"
        output_file_name = "path/to/output_file.csv"
        ...
        with patch("pandas.read_csv", pd_mock):
            with patch("pandas.DataFrame.to_csv", to_csv_mock):
                transformed_file_name = file_finder.transform_file
